...
def main():
    print("T E A M    N E I M A N   ! !")
    print("Hello!! Welcome to NeimanBot Session Generator\n")
...
def generate_pyro_session():
    ...
    APP_ID = int(input("\nEnter APP ID here: "))
    API_HASH = input("\nEnter API HASH here: ")
    with PClient(name="neimanuser", api_id=APP_ID, api_hash=API_HASH, in_memory=True) as neimanbot:
        print("\nYour NeimanBot Session Is sent in your Telegram Saved Messages.")
        neimanbot.send_message(
            "me",
            f"#NEIMANBOT #PYROGRAM\n\n`{neimanbot.export_session_string()}`",
        )


def generate_telethon_session():
    print("\nTelethon Session For NeimanBot!")
    APP_ID = int(input("\nEnter APP ID here: "))
    API_HASH = input("\nEnter API HASH here: ")
    with TelegramClient(StringSession(), APP_ID, API_HASH) as neimanbot:
        print("\nYour NeimanBot Session Is sent in your Telegram Saved Messages.")
        neimanbot.send_message(
            "me",
            f"#NEIMANBOT #TELETHON \n\n`{neimanbot.session.save()}`",
        )
...
def neimanbot_session(session):
    ...
    if len(session) in pyro_format.keys():
        ...
        return f"=={neimanbot('neiman')}{new_session}{neimanbot('bot')}=="
    else:
        return error_msg
